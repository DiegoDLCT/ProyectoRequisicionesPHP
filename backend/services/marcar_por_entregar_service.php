...
if (!isset($_SESSION['usuario'])) {
    ...
    header('Location: /ProyectoPHP/frontend/views/pagos/gestionar_entregas.php');
    exit();
}
...
try {
    ...
    // Cambiar estado de pagado a por_entregar
    $query = "UPDATE requisiciones SET estado = 'por_entregar' WHERE id = :id AND estado = 'pagado'";
    $stmt = $db->prepare($query);
    $stmt->bindParam(':id', $id_requisicion);
    
    if ($stmt->execute()) {
        $_SESSION['mensaje_exito'] = 'Requisición marcada como por entregar';
    } else {
        $_SESSION['mensaje_error'] = 'Error al marcar la requisición';
    }
} catch (Exception $e) {
